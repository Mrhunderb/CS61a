...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def select(words):
        words = set(split(lower(remove_punctuation(words))))
        for s in topic:
            if s in words:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    similar = min(word_list, key = lambda w: diff_function(typed_word, w, limit))
    if diff_function(typed_word, similar, limit) > limit:
        return typed_word
    else:
        return similar 
    # END PROBLEM 5
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    pro = 0
    for w1, w2 in zip(sofar, prompt):
        if w1 == w2:
            pro += 1
        else:
            break
    progress = pro / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        ...
        for i in range(len(player) - 1):
            time_player.append(player[i+1] - player[i])
        time_diff.append(time_player)
...
def fastest_words(match):
    ...
    "*** YOUR CODE HERE ***"
    """
    total = len(match["times"])
    fastest = [[] for i in range(total)]
    for i in range(total):
        f = min(time(match, i, x) for x in word_indices)
        fastest[i].append(word_at(match, f))
    return fastest    
    """
    fastest = [[] for i in player_indices]
    small_time = []
    for i in word_indices:
        small_time = [time(match, x, i) for x in player_indices] 
        index = min(player_indices, key = lambda x: small_time[x])
        fastest[index].append(word_at(match, i))
    return fastest
    # END PROBLEM 10

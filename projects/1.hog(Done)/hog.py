...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    summ = 0
    if num_rolls == 0:
       summ = hefty_hogs(player_score, opponent_score)
    else:
       summ = roll_dice(num_rolls, dice)
    return summ
    # END PROBLEM 3
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    a = make_averaged(roll_dice, total_samples)
    call = [a(i, dice) for i in range(1, 11)]
    return call.index(max(call)) + 1    
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    p = hefty_hogs(score, opponent_score) 
    score += hefty_hogs(score, opponent_score) 
    q = hog_pile(score, opponent_score)
    if p >= threshold:
       return 0
    elif q != 0: 
       return 0
